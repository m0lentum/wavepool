...
class StandingWave(Simulation):
    def __init__(self):
        ...
        # multiplier matrix for velocity in the time-stepping formula for pressure
        p_step_mat = (
        ...
        for bound_edge in cmp_complex.boundary():
            for bound_vert in bound_edge.boundary():
                ...
                p_step_mat[vert_idx, :] = 0.0
        # multiplier matrix for pressure in the time-stepping formula for velocity
        v_step_mat = dt * cmp_complex[0].d

        self.p_step_mat = p_step_mat
    ...
    def init_state(self):
        x_wave_count = 2
        y_wave_count = 3
        self.p = np.sin(x_wave_count * self.complex.vertices[:, 0]) * np.sin(
        ...
        self.v = np.zeros(cmp_complex[1].num_simplices)

    def step(self):
        self.p += self.p_step_mat * self.v
        self.v += self.v_step_mat * self.p
...
class MovingWave(StandingWave):
    def init_state(self):
        ...
        self.p = np.multiply(0.2 * np.multiply(x, x), np.sin(3 * x)) * np.sin(y)
        self.v = np.zeros(cmp_complex[1].num_simplices)


sim1 = StandingWave()
sim1_vis = anim.PressureOnVertices(sim=sim1, get_data=lambda s: s.p, zlim=[-1.5, 1.5])
sim1_vis.show()

sim2 = MovingWave()
sim2_vis = anim.PressureOnVertices(sim=sim2, get_data=lambda s: s.p, zlim=[-1.5, 1.5])
sim2_vis.show()
# sim2_vis.save()
